...
def _format_value(value: float | str | bool | None) -> str:  # noqa: FBT001
    """Format a metric value as a string suitable for a LaTeX table cell."""
    if value is None:
        return "--"
...
def run_comparative_analysis(analysis_paths: dict[str, Path], output_path: Path) -> None:  # noqa: PLR0915
    ...
    for model_name, experiment_name in zip(model_names, experiment_names, strict=True):
        ...
        for json_path in sorted(analysis_path.glob("*.json")):
            ...
            if not isinstance(data, dict):
                continue
            for metric_name, metric_value in _flatten_metrics(data).items():
    ...
    with table_path.open("w", encoding="utf-8") as handle:
        ...
        handle.write("\\toprule\n")
        handle.write(f" {' & '.join([f'\\textbf{{{cell}}}' for cell in header_cells])} \\\\\n")
        ...
        sorted_files = sorted(file_to_metrics.keys())
        for file_name in sorted_files:
            ...
            for idx, metric_name in enumerate(metrics):
                ...
                if idx == 0:
                    ...
                    file_cell = ""
                row_values = [file_cell, _escape_latex(metric_map.get(metric_name, metric_name))]
                row_values.extend([_escape_latex(_format_value(values.get(model_name))) for model_name in model_names])
                handle.write(f" {' & '.join(row_values)} \\\\\n")

            # Add midrule between file groups, but not after the last one
            if file_name != sorted_files[-1]:
                handle.write("\n\\midrule\n")
