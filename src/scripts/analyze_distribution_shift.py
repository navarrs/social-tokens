...
def _get_scores_from_h5(group: h5py.File) -> tuple[NDArray[np.int_], NDArray[np.int_]]:
    """Extract individual and interaction agent scores from an h5py group.

    Args:
        group (h5py.File): The h5py group to extract scores from.
    ...
    for key in group:
        if "individual_agent_scores" not in group[key] or "interaction_agent_scores" not in group[key]:  # pyright: ignore[reportOperatorIssue]
            continue
        individual_scores = group[key]["individual_agent_scores"][:].squeeze(-1).squeeze(-1)  # pyright: ignore[reportIndexIssue, reportAttributeAccessIssue]
        interaction_scores = group[key]["interaction_agent_scores"][:].squeeze(-1).squeeze(-1)  # pyright: ignore[reportIndexIssue, reportAttributeAccessIssue]
        full_individual_scores.append(individual_scores)
        full_interaction_scores.append(interaction_scores)
    return np.concatenate(full_individual_scores, axis=0).astype(int), np.concatenate(
        full_interaction_scores, axis=0
    ).astype(int)


def _compute_heatmap(h5_files: list[Path]) -> NDArray[np.int_]:
    ...
    for h5_file in h5_files:
        ...
        with h5py.File(h5_file, "r") as group:
            individual_scores, interaction_scores = _get_scores_from_h5(group)
            full_individual_scores.append(individual_scores)
            full_interaction_scores.append(interaction_scores)
...
def analyze_distribution_shift(data_cache_path: Path, data_subsets: list[str], output_path: Path) -> None:
    """Analyze distribution shift in datasets by generating heatmaps of agent scores.
    ...
    for data_subset in data_subsets:
        ...
        heatmap = _compute_heatmap(h5_files)
...
if __name__ == "__main__":
    ...
    analyze_distribution_shift(args.data_cache_path, args.data_subsets, args.output_path)
